...
```php
generateAddress($blockchain, $network, $context, $generate_address_rb): \CryptoAPIs\Model\GenerateAddressR
```
...
$context = 'context_example'; // string | In batch situations the user can use the context to correlate responses with requests. This property is present regardless of whether the response was successful or returned as an error. `context` is specified by the user.
$generate_address_rb = new \CryptoAPIs\Model\GenerateAddressRB(); // \CryptoAPIs\Model\GenerateAddressRB

try {
    $result = $apiInstance->generateAddress($blockchain, $network, $context, $generate_address_rb);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling FeaturesApi->generateAddress: ', $e->getMessage(), PHP_EOL;
}
```
 ...
 **network** | **string**| Represents the name of the blockchain network used; blockchain networks are usually identical as technology and software, but they differ in data, e.g. - \&quot;mainnet\&quot; is the live network with actual data while networks like \&quot;testnet\&quot;, \&quot;ropsten\&quot; are test networks. |
 **context** | **string**| In batch situations the user can use the context to correlate responses with requests. This property is present regardless of whether the response was successful or returned as an error. &#x60;context&#x60; is specified by the user. | [optional]
 **generate_address_rb** | [**\CryptoAPIs\Model\GenerateAddressRB**](../Model/GenerateAddressRB.md)|  | [optional]

### Return type

[**\CryptoAPIs\Model\GenerateAddressR**](../Model/GenerateAddressR.md)
